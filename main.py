    ...
    def _wait_until_next_candle(self) -> None:
        ...
        try:
            ...
            for target in [15, 30, 45, 0]:
                if target == 0:
                    target_time = current.replace(minute=0, second=0, microsecond=200000) + datetime.timedelta(hours=1)
                    break
                if minute < target:
                    target_time = current.replace(minute=target, second=0, microsecond=200000)
                    break
